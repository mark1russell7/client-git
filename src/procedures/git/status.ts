...
export async function gitStatus(input: GitStatusInput): Promise<GitStatusOutput> {
  ...
  for (const line of statusOutput.split("\n").filter(Boolean)) {
    ...
    if (index === "?" && worktree === "?") {
      ...
      if (index === "A") { status = "added"; staged = true; }
      else if (index === "D") { status = "deleted"; staged = true; }
      else if (index === "R") { status = "renamed"; staged = true; }
      else if (index === "C") { status = "copied"; staged = true; }
      else if (index === "M") { status = "modified"; staged = true; }
      else if (worktree === "M") { status = "modified"; }
      else if (worktree === "D") { status = "deleted"; }
      else if (worktree === "A") { status = "added"; }
    }
